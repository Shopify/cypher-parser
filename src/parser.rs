...
impl Parser {
    ...
    fn parse_usize(&mut self) -> Result<usize, CypherError> {
        match self.advance() {
            Some(Token {
                ...
                .map_err(|_| CypherError::syntax("expected a non-negative integer", position)),
            _ => Err(CypherError::syntax(
                "expected an integer",
                self.current_position(),
            )),
    ...
    fn parse_var_length(&mut self) -> Result<VarLength, CypherError> {
        ...
        if let Some(TokenKind::Int(value)) = self.peek_kind() {
            let lower = u32::try_from(*value).map_err(|_| {
                CypherError::syntax(
                    "variable-length bound is too large",
                    self.current_position(),
                )
            })?;
            self.position += 1;
    ...
    fn parse_optional_length_bound(&mut self) -> Result<Option<u32>, CypherError> {
        if let Some(TokenKind::Int(value)) = self.peek_kind() {
            let upper = u32::try_from(*value).map_err(|_| {
                CypherError::syntax(
                    "variable-length bound is too large",
                    self.current_position(),
                )
